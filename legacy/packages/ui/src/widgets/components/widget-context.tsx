...
import type { WidgetDefinition, WidgetSize, WidgetBreakpoint } from "./widget-registry"
import { createWidgetStore, type WidgetStore } from "./widget-store"
import type { StoreApi } from "zustand"

interface WidgetContextValue {
  registerWidget: <TProps>(type: string, definition: WidgetDefinition<TProps>) => void
...
export function WidgetProvider({ 
  ...
  const registry = useMemo(() => new Map(Object.entries(initialWidgets)), [initialWidgets])
  
  // Create a new store instance for this provider
  const store = useMemo(() => createWidgetStore(storeId), [storeId])

  const registerWidget = useCallback(<TProps,>(type: string, definition: WidgetDefinition<TProps>) => {
    registry.set(type, definition as WidgetDefinition)
  }, [registry])

  const getWidget = useCallback((type: string) => {
