import { EventStream } from "@/features/event-stream/client";
...
import { Folder, type FolderId } from "../../../domain/files-rpc";
...
const makeApiMock = (options?: {
  ...
  const layer = Layer.mock(Api, {
    _tag: "@features/files/client/-files/files-atoms/Api",
    list: () => {
...
const makeFilePickerMock = (file: File | null) =>
  Layer.mock(FilePicker, {
    _tag: "@features/files/client/-files/files-atoms/FilePicker",
...
const makeFileSyncMock = () => {
  ...
  const layer = Layer.mock(FileSync, {
    _tag: "@features/files/client/-files/files-atoms/FileSync",
...
const makeEventStreamMock = () => {
  ...
  const layer = Layer.mock(EventStream, {
    _tag: "@features/event-stream/client/EventStream",
