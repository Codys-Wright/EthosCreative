...
import type { UserId } from "@/features/auth/policy";
import type { EventStreamEvents } from "../domain/event-stream-rpc.js";
...
export class EventStreamHub extends Effect.Service<EventStreamHub>()("EventStreamHub", {
  scoped: Effect.gen(function* () {
    ...
    ): Effect.Effect<void, never, CurrentUser> =>
      CurrentUser.pipe(Effect.flatMap((user) => notifyUser(user.userId, event)));
