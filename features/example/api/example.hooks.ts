...
import { NewExample, ExampleType, NewExampleType } from "./../types/example.type";
...
export namespace ExampleHooks {
  ...
  export const exampleQueryKey = createQueryKey<"example", ExampleVars>(
    "example",
  );
    ...
    return useEffectMutation({
      ...
      mutationFn: ({ id, data }: { id: string; data: Partial<NewExampleType> }) => {
        ...
        exampleData.setData({}, (draft) => {
          const exampleList = draft as ExampleType[];
          const exampleIndex = exampleList.findIndex(
            (example) => String(example.id) === String(id),
          );
          if (exampleIndex !== -1) {
            ...
            E.sync(() => {
              ...
              exampleData.setData({}, (draft) => {
                ...
                if (exampleIndex !== -1 && updatedExample) {
                  exampleList[exampleIndex] = updatedExample as ExampleType;
                }
              });
  ...
  export const useCreate = () => {
    return useEffectMutation({
      ...
      mutationFn: (input: NewExampleType) => {
        ...
        const optimisticExample: ExampleType = {
          ...input,
          ...
          E.tap((serverResponse) =>
            E.sync(() => {
              ...
              if (
                ...
                exampleData.setData({}, (draft) => {
                  const exampleList = draft as ExampleType[];
                  const tempIndex = exampleList.findIndex(
                    (ex) => String(ex.id) === tempId,
                  );
                  if (tempIndex !== -1) {
  ...
  export const useDelete = () => {
    ...
    return useEffectMutation({
      ...
      mutationFn: (id: string) => {
        ...
        const currentExample = queryClient.getQueryData<ExampleType>(
          exampleQueryKey({ id }),
        );
        ...
        exampleData.setData({}, (draft) => {
          const exampleList = draft as ExampleType[];
          const index = exampleList.findIndex(
            (example) => String(example.id) === id,
          );
          if (index !== -1) {
  ...
  export const useUndoDelete = () => {
    return useEffectMutation({
      ...
      mutationFn: (id: string) => {
        return ExampleService.undoDelete(id).pipe(
          E.tap((response) => 
            E.sync(() => {
              exampleData.invalidateAllQueries();
              
              toast.success("Example restored", {
                description: "The example has been successfully restored",
                id: `restore-example-${id}-success`,
              });
            })
          ),
          E.catchAll((error) => {
            toast.error("Failed to restore example", {
              description: String(error.message || "An unexpected error occurred"),
              id: `restore-example-${id}-error`,
            });
