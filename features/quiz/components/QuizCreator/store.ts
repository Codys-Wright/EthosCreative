...
interface QuizStore {
  ...
  addQuestion: (
    question:
      | Omit<RatingQuestion, "id">
      | Omit<MultipleChoiceQuestion, "id">
      | Omit<TrueFalseQuestion, "id">
      | Omit<ShortAnswerQuestion, "id">,
  ) => void;
...
export const useQuizStore = create<QuizStore>((set, get) => ({
  ...
  currentQuestionId: "1",
  setQuizData: (quiz) =>
    set({ quiz, currentQuestionId: quiz.questions[0]?.id ?? null }),
    ...
    set((state) => ({
      quiz: {
        ...
        questions: [
          ...
          { ...question, id: Math.random().toString(36).slice(2) } as Question,
        ],
      },
    })),
