...
export class ArtistTypeAnalysisEngine implements AnalysisEngine {
  ...
  description =
    "Analyzes quiz responses to determine artist type based on ideal answer ranges";
  ...
  calculatePoints(value: number, idealAnswer: IdealAnswer): number {
    ...
    if (idealAnswer.values.includes(value)) {
      return idealAnswer.primary
        ? SCORING_CONFIG.PRIMARY_IDEAL_POINTS
        : SCORING_CONFIG.SECONDARY_IDEAL_POINTS;
    }
    ...
    const points =
      SCORING_CONFIG.SECONDARY_IDEAL_POINTS -
      SCORING_CONFIG.POINT_FALLOFF * distance;

    // Only apply minimum points if MIN_POINTS is not null
    return SCORING_CONFIG.MIN_POINTS !== null
      ? Math.max(points, SCORING_CONFIG.MIN_POINTS)
      : points;
  }

  getCurrentPoints(
    responses: { questionId: string; response: number | null }[],
  ): Record<ArtistType, number> {
    ...
    responses.forEach((response) => {
      ...
      const analysis = this.questionAnalysis.find(
        (qa) => qa.questionId === response.questionId,
      );
      if (!analysis) {
        return;
      }
  ...
  analyze(
    ...
    results.forEach((result) => {
      result.percentage =
        totalPoints > 0 ? (result.points / totalPoints) * 100 : 0;
    });
  ...
  private generateQuestionAnalysis(questionIds: string[]): QuestionAnalysis[] {
    const analysis = questionIds.map((questionId) => {
      ...
      Object.keys(artistTypeIdealAnswers).forEach((artistType) => {
        const idealAnswer = getIdealAnswer(
