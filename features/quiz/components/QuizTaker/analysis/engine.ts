...
export function calculateDistancePoints(
  ...
  const points =
    SCORING_CONFIG.SECONDARY_IDEAL_POINTS -
    distance * SCORING_CONFIG.POINT_FALLOFF;
...
export class DefaultAnalysisEngine implements AnalysisEngine {
  ...
  description =
    "Analyzes quiz responses to determine artist type based on response patterns";
  ...
  analyze(responses: QuizResponse[]): QuizAnalysisResult {
    ...
    const results = Object.entries(points).map(([type, points]) => ({
      ...
      points,
      percentage:
        totalPoints > 0 ? (Math.max(0, points) / totalPoints) * 100 : 0,
    }));
