...
export const useQuizTakerStore = create<QuizTakerState>((set, get) => ({
  ...
  setResponse: (questionId, response) => {
    ...
    const isFirstAnswer =
      get().responses.find((r) => r.questionId === questionId)?.response ===
      null;
    ...
    set((state) => {
      ...
      const updatedQuestionAnalytics = state.analytics.questionAnalytics.map(
        (qa) => {
          if (qa.questionId === questionId) {
            return {
              ...qa,
              answerChanges: isChangingAnswer
                ? qa.answerChanges + 1
                : qa.answerChanges,
            };
          }
  ...
  nextQuestion: () => {
    ...
    if (
      ...
      get().canAdvanceToIndex(nextIndex)
    ) {
